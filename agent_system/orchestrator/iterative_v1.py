...
class IterativeSolver:
    ...
    def _make_state_prompt(
        ...
        events = self.kb.read_events_by_type(event_type="slm_done")
        histories = [
            f"{e['step']}: model answer {e['slm_res']} is {e['verified']}"
            for e in events
        ]
        histories_str = "\n".join(histories)
        return (
    ...
    def run(
        ...
        while it < self.max_iter:
            ...
            if not plan:
                ...
                logger.info(
                    f"[ROOT] replan #{replans_used} state_prompt:\n{state_prompt}"
                )
            ...
            logger.info(f"[{step_id}] SLM exec_question:\n{exec_question}")
            ...
            v_root = self.supervisor.verify_final(
                root_question,
                candidate,
                self.tools,
                self.kb,
            ...
            if v_root.get("verdict") == "final_correct":
                ...
                successes.append({"subtask": step, "answer": final_answer})
                self.kb.append(
                    {
                        "event": "root_final",
                        "root_id": root_id,
            ...
            # 4) 중간단계 평가 -> 성공 시 다음 step 수행
            logger.info(f"[{step_id}] LLM verify step ...")
            v = self.supervisor.verify_intermediate(
                root_question,
                step,
                slm_answer,
                self.tools,
