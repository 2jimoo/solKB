...
class LLMSupervisor:
    ...
    def decompose(self, question: str, depth: int, tools: ToolRegistry, kb: JsonlKB, node_id: str) -> List[str]:
        schema = {
            "type": "json_schema",
            "json_schema": {
                "name": "decomposition",
                "schema": {
                    "type": "object",
                    "properties": {
                        "subtasks": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 10}
                    },
                    "required": ["subtasks"],
        ...
        messages = [
            ...
            {"role": "user", "content": f"Depth={depth}\nTask:\n{question}\nReturn 3-10 subtasks."},
        ]
        resp = self.runner.run(messages, tools, kb=kb, node_id=node_id, label="llm_decompose", response_format=schema)
    ...
    def verify(
        ...
        schema = {
            ...
            "json_schema": {
                ...
                "schema": {
                    "type": "object",
                    "properties": {
                        "verdict": {"type": "string", "enum": ["correct", "incorrect", "insufficient"]},
                        "reason": {"type": "string"},
        ...
        resp = self.runner.run(messages, tools, kb=kb, node_id=node_id, label="llm_verify", response_format=schema)
    ...
    def synthesize(
        ...
        resp = self.runner.run(messages, tools, kb=kb, node_id=node_id, label="llm_synthesize", response_format=None)
